...
// TEMPORARY: one-time dev→prod migration. Remove after use.

importRouter.post("/db-restore", upload.single("data"), async (req, res) => {
  if (!req.file) { res.status(400).json({ error: "No file uploaded" }); return; }
  ...
  await db.monzoApiTransaction.deleteMany();
  await db.amexTransaction.deleteMany();
  await db.barclaysTransaction.deleteMany();
  await db.santanderTransaction.deleteMany();
  await db.hsbcTransaction.deleteMany();
  await db.chaseTransaction.deleteMany();
  await db.sofiTransaction.deleteMany();
  await db.plaidTransaction.deleteMany();
  ...
  const insertMany = async (table: string, create: (rows: any[]) => Promise<{ count: number }>) => {
    const rows = data[table] ?? [];
    if (rows.length > 0) {
      const { count } = await create(rows as any[]);
      counts[table] = count;
    }
  };

  await insertMany("Category",            (r) => db.category.createMany({ data: r }));
  await insertMany("InvestmentAccount",   (r) => db.investmentAccount.createMany({ data: r }));
  await insertMany("Transaction",         (r) => db.transaction.createMany({ data: r }));
  await insertMany("InvestmentSnapshot",  (r) => db.investmentSnapshot.createMany({ data: r }));
  await insertMany("Note",                (r) => db.note.createMany({ data: r }));
  await insertMany("Tab",                 (r) => db.tab.createMany({ data: r }));
  await insertMany("PlaidItem",           (r) => db.plaidItem.createMany({ data: r }));
  await insertMany("PlaidTransaction",    (r) => db.plaidTransaction.createMany({ data: r }));
  await insertMany("MonzoApiTransaction", (r) => db.monzoApiTransaction.createMany({ data: r }));
  await insertMany("AmexTransaction",     (r) => db.amexTransaction.createMany({ data: r }));
  await insertMany("BarclaysTransaction", (r) => db.barclaysTransaction.createMany({ data: r }));
  await insertMany("SantanderTransaction",(r) => db.santanderTransaction.createMany({ data: r }));
  await insertMany("HsbcTransaction",     (r) => db.hsbcTransaction.createMany({ data: r }));
  await insertMany("ChaseTransaction",    (r) => db.chaseTransaction.createMany({ data: r }));
  await insertMany("SofiTransaction",     (r) => db.sofiTransaction.createMany({ data: r }));
