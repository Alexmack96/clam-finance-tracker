...
function plaidGuard(res: import("express").Response): boolean {
  if (!env.PLAID_CLIENT_ID || !env.PLAID_SECRET) {
    res.status(503).json({ error: "Plaid not configured — set PLAID_CLIENT_ID and PLAID_SECRET in .env" });
    return false;
  }
  return true;
}
...
plaidRouter.get("/status", requireAuth, requireAdmin, async (req, res) => {
...
plaidRouter.post("/link-token", requireAuth, requireAdmin, async (req, res) => {
...
plaidRouter.post("/exchange", requireAuth, requireAdmin, async (req, res) => {
  if (!plaidGuard(res)) return;
  ...
  await db.plaidItem.upsert({
    where:  { userId: req.user!.id },
    create: { id: randomBytes(8).toString("hex"), userId: req.user!.id, accessToken: access_token, itemId: item_id, updatedAt: new Date() },
...
plaidRouter.post("/sync", requireAuth, requireAdmin, async (req, res) => {
  if (!plaidGuard(res)) return;

  const item = await db.plaidItem.findUnique({ where: { userId: req.user!.id } });
  if (!item) { res.status(400).json({ error: "Santander not connected — link your account first" }); return; }
  ...
  if (added.length === 0) { res.json({ imported: 0, duplicates: 0 }); return; }

  const ids = added.map((tx) => tx.transaction_id);
  const existing = await db.plaidTransaction.findMany({ where: { transactionId: { in: ids } }, select: { transactionId: true } });
...
plaidRouter.post("/disconnect", requireAuth, requireAdmin, async (req, res) => {
  const item = await db.plaidItem.findUnique({ where: { userId: req.user!.id } });
  if (item) {
    try {
      const client = makePlaidClient();
