...
/// Uploads a Santander current-account statement PDF: parse it, prove it, keep
/// it, stage its rows.
///
...
public sealed class ImportSantanderCommand(
    IDbConnectionFactory factory,
    IStatementStore store,
    IIdGenerator ids,
    TimeProvider clock)
{
    private const string Bank = "santander";
    ...
    /// Only the rows that have an id. The column is nullable because Santander
    ...
    private const string ExistingIdsSql =
        "SELECT [transactionId] FROM [SantanderTransactions] WHERE [transactionId] IS NOT NULL;";
    ...
    public async Task<Result<ImportSantanderResponse>> ExecuteAsync(
        ImportSantanderRequest request,
        CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(request);

        if (request.File is null || request.File.Length == 0)
            return Result<ImportSantanderResponse>.Invalid(new ValidationError("file", "No file uploaded"));
        ...
        if (existingFile is not null)
        {
            ...
            return Result<ImportSantanderResponse>.Conflict(
        ...
        using (var transaction = connection.BeginTransaction())
        {
            ...
            if (duplicates.Count > 0)
            {
                await connection.ExecuteAsync(new CommandDefinition(ClaimOrphanedSql, new
                {
                    ...
                    ExternalIds = duplicates.Select(d => $"santander:{d}").ToList(),
                }, transaction, cancellationToken: ct));
            }
        ...
        return Result<ImportSantanderResponse>.Success(new ImportSantanderResponse
        {
    ...
    private static Result<ImportSantanderResponse> Rejected(SantanderParseResult parsed) => parsed.Status switch
    {
        StatusCodes.Status400BadRequest =>
            Result<ImportSantanderResponse>.Invalid(new ValidationError("file", parsed.Error!)),
        StatusCodes.Status409Conflict => Result<ImportSantanderResponse>.Conflict(parsed.Error!),
        _ => Result<ImportSantanderResponse>.Error(parsed.Error!),
    };
