...
/// the Amex and HSBC equivalents: a PDF that cannot be read (400) is a different
...
public sealed record BarclaysParseResult
{
    private BarclaysParseResult() { }

    public bool Ok { get; private init; }

    public IReadOnlyList<BarclaysRow> Rows { get; private init; } = [];
    ...
    public static BarclaysParseResult Parsed(IReadOnlyList<BarclaysRow> rows, string statementDate) =>
        new() { Ok = true, Rows = rows, StatementDate = statementDate };

    public static BarclaysParseResult Rejected(int status, string error) =>
