...
/// Uploads a SoFi Money statement PDF: parse it, prove it, keep it, stage its
/// rows.
///
...
///
/// One PDF stages two accounts' rows, Checking and Savings, against the one
/// statement file. That is right: it is one document, and the transfers between
/// the two accounts only make sense read together.
///
...
public sealed class ImportSofiCommand(
    IDbConnectionFactory factory,
    IStatementStore store,
    IIdGenerator ids,
    TimeProvider clock)
{
    private const string Bank = "sofi";
    ...
    private const string ExistingIdsSql = "SELECT [transactionId] FROM [SofiTransactions];";
    ...
    public async Task<Result<ImportSofiResponse>> ExecuteAsync(
        ImportSofiRequest request,
        CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(request);

        if (request.File is null || request.File.Length == 0)
            return Result<ImportSofiResponse>.Invalid(new ValidationError("file", "No file uploaded"));
        ...
        if (existingFile is not null)
        {
            ...
            return Result<ImportSofiResponse>.Conflict(
        ...
        using (var transaction = connection.BeginTransaction())
        {
            ...
            if (duplicates.Count > 0)
            {
                await connection.ExecuteAsync(new CommandDefinition(ClaimOrphanedSql, new
                {
                    ...
                    ExternalIds = duplicates.Select(d => $"sofi:{d}").ToList(),
                }, transaction, cancellationToken: ct));
            }
        ...
        return Result<ImportSofiResponse>.Success(new ImportSofiResponse
        {
    ...
    private static Result<ImportSofiResponse> Rejected(SofiParseResult parsed) => parsed.Status switch
    {
        StatusCodes.Status400BadRequest =>
            Result<ImportSofiResponse>.Invalid(new ValidationError("file", parsed.Error!)),
        StatusCodes.Status409Conflict => Result<ImportSofiResponse>.Conflict(parsed.Error!),
        _ => Result<ImportSofiResponse>.Error(parsed.Error!),
    };
